...
"""
Takes each original SGEMS grid, copies it  to the large grid (large enough for MODFLOW sims).
Then runs snesim with presimulated data = copied grid data.
...
#grid details
import_dir = 'E:/SGEMS/'
import_prefix = ['snesimPosteriorJ__170206','snesimPosteriorJ__170212','snesimPosteriorJ__170212']
sim_prefix = ['snesim_170206_posteriorJ','snesim_170212_posteriorJ','snesim_170212_posteriorJ']
suffixes = [55,65,75]
export_name = 'snesimPosteriorTkmeans'
...
for i in range(len(pSand)):
      if i in skipglobal:
          ...
          smallgrid_name = import_prefix[i] + '_' + str(suffixes[i])
          ...
          for j in range(int(reals[i])):
                ...
                sim_name = sim_prefix[i] + '_' + str(suffixes[i]) + '_' + str(simcount)
