# -*- coding: utf-8 -*-
"""
Created on Thu Dec 27 16:06:37 2018
...
def compute_export_hausdorff(dirname,saveyn=1):
    ...
    try:
        ...
        conc_mat = np.zeros((len(conc_fnames),26,20,100),dtype=float)
    ...
    hdorf_list= []
    #hdorf_list = np.zeros((int(len(idx_dict_filt)*(len(idx_dict_filt)-1)/2)))
    csvfile = str(dirname.joinpath('hausdorff.csv'))
    fcsv = open(csvfile,mode='w')
    i=0
    while i < len(idx_dict_filt):
        for j in range(i+1,len(idx_dict_filt)):
            val = mod_hausdorff(idx_dict_filt[j],idx_dict_filt[i])
            #hdorf_list.append(val)
            hdorf_mat[i,j] = val
            fcsv.write(str(val) + '\n')
            #val = mod_hausdorff(idx_dict_filt[j],idx_dict_filt[i])
            print('row' ,i,' col ',j)
